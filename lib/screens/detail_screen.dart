...
class _DetailScreenState extends State<DetailScreen> {
  ...
  // setiap kali diubah oleh setState flutter akan melakukan rebuild, supaya tampilan ikut terupdate
  int _quantity = 1; 

  void _increment() => setState(() => _quantity++);

  void _dicrement() { // supaya pengurangan hanya bisa dilakukan kalau jumlah lebih dari 1
    if (_quantity > 1) { 
      setState(() => _quantity--);
    }
  } 
  ...
  void _addToCart() {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text('$_quantity x ${flower.name} ditambahkan ke kerangajang')
        )
    );
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
              ...
              child: SingleChildScrollView(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    SizedBox(height: 26),
                    QuantityStepper(quantity: _quantity, onIncrement: _increment, onDecrement: _dicrement),
                  ],
                ),
              ),
            )
      ...
      bottomNavigationBar: DetailTotal(totalPrice: flower.price * _quantity),
    );
  }
}